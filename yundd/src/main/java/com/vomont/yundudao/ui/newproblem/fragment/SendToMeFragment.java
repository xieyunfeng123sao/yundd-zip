...
public class SendToMeFragment extends Fragment implements INewProblemView
{
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState)
    {
        ...
        problem_tome_list.setOnRefreshListener(new OnRefreshListener2<ListView>()
        {
            ...
            {
                page = 1;
                presenter.getProblemList(shareUtil.getShare().getUserid() + "", 0, 0, shareUtil.getShare().getAccountid());
            }
            ...
            {
                page++;
                presenter.getProblemList(shareUtil.getShare().getUserid() + "", mlist.get(mlist.size()-1).getProblemid(), 0, shareUtil.getShare().getAccountid());
            }
...
//                actionSheetDialog.setTitle("请选择以下操作");
                if (state == 3)
                {
                    ...
                    actionSheetDialog.addSheetItem("无需整改", SheetItemColor.Blue, new OnSheetItemClickListener()
                    {
                        @Override
                        public void onClick(int which)
                        {
                            action_result = 3;
                            startIntentActvity(3, 3, problemid, position,"无需整改");
                        }
                    });
                }
                else if (state == 4)
