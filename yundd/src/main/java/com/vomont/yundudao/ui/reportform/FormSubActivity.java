...
import com.vomont.yundudao.ui.reportform.adapter.ForSubAdapter;
import com.vomont.yundudao.utils.ShareUtil;

public class FormSubActivity extends Activity implements IFactoryView {
	...
	private void initView() {
		...
		jump.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
				if (adapter.getSelectedSub() != null
						&& adapter.getSelectedSub().size() != 0) {
					Intent intent = getIntent();
					intent.putExtra("subfatory",
							(Serializable) adapter.getSelectedSub());
					setResult(RESULT_OK, intent);
					finish();
				} else {
					Toast.makeText(FormSubActivity.this, "选择巡视点",
	...
	public void getFactory(FactoryBean factoryBean) {
		if (factoryBean != null) {
			...
			adapter = new ForSubAdapter(this, mlist);
